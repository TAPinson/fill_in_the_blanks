...
blank_four = "___4___"
blank_five = "___5___"
blanks = [blank_one, blank_two, blank_three, blank_four, blank_five]
...
def game_engine(quiz,answers):
    ...
    if response == answers[0]:
        ...
        quiz = [blank.replace(blank_four, response) for blank in quiz]
        print ' '.join(quiz)
        return next_level()
    else:
...
def opening_prompt(quiz,answers):
    response = raw_input('Which level would you like to play?(easy/medium/hard/custom): ')
    if response == 'easy':
        ...
        with open ("easy-answers.txt","r") as answers:
            ...
            with open ("medium-quiz.txt", "r") as quiz:
                quiz = quiz.read().replace('\n', '')
                quiz = quiz.split()
                print ' '.join(quiz)
            with open ("medium-answers.txt","r") as answers:
                answers = answers.read().replace ('\n', '')
                answers = answers.split()
                return game_engine(quiz,answers)
    elif response == 'hard':
            with open("hard-quiz.txt", "r") as quiz:
                quiz = quiz.read().replace('\n', '')
                quiz = quiz.split()
                print ' '.join(quiz)
            with open("hard-answers.txt", "r") as answers:
                answers = answers.read().replace('\n', '')
                answers = answers.split()
                return game_engine(quiz, answers)
    else:
